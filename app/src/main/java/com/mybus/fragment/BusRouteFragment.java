...
public class BusRouteFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        mStopOrigin.setText(mBusRouteResult.getBusRoutes().get(0).getStartBusStopStreetName()
                + " "
                + mBusRouteResult.getBusRoutes().get(0).getStartBusStopStreetNumber());
        mWalkOrigin.setText(getContext().getString(R.string.distance_to_origin)
        ...
        if (mBusRouteResult.getType() == 0) {
            mStopDestination.setText(mBusRouteResult.getBusRoutes().get(0).getDestinationBusStopStreetName()
                    + " "
                    + mBusRouteResult.getBusRoutes().get(0).getDestinationBusStopNumber());
            mWalkDestination.setText(getContext().getString(R.string.distance_to_destination)
                    + WalkDistanceHelper.getDistanceInBlocks(
                    mBusRouteResult.getBusRoutes().get(0).getDestinationBusStopDistanceToDestination()));
        } else {
            mStopDestination.setText(mBusRouteResult.getBusRoutes().get(1).getDestinationBusStopStreetName()
                    + " "
                    + mBusRouteResult.getBusRoutes().get(1).getDestinationBusStopNumber());
            mWalkDestination.setText(getContext().getString(R.string.distance_to_destination)
                    + WalkDistanceHelper.getDistanceInBlocks(
